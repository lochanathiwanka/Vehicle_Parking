...
public class CarOwnerPaymentCheckFormController implements Initializable {
    public TextField txtCarNumberField;
    ...
    public void btnRemoveOnAction(ActionEvent actionEvent) {
        ...
        vehicle.setSQL("Delete from carParkingTempory where number = ?");
        try{
            boolean isRemoved = VehicleController.removeVehicleFromParkingTempory(vehicle,txtCarNumberField.getText());
            if (isRemoved){
                JOptionPane.showMessageDialog(null,"Car was removed from parking","",JOptionPane.INFORMATION_MESSAGE);
                textFieldsReset();
            }
    ...
    public void textFieldsReset(){
        txtCarNumberField.setText(null);
    ...
    public void forSearchField(){
        Vehicle vehicle = new Car();
        vehicle.setSQL("Select number,firstName,lastName,paymentStatus from carParkingTempory where number = ?");
        try {
            String[] list = VehicleController.getAllValuesFromParkingHistory(vehicle,txtSearch.getText());
            if (list!=null){
                txtCarNumberField.setText(list[0]);
