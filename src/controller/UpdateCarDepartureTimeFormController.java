...
public class UpdateCarDepartureTimeFormController {
    ...
    public void btnAddOnAction(ActionEvent actionEvent) {
        ...
        vehicle.setSQL("Update carParkingTempory set departureTime=? where number=?");
        try {
            ...
            boolean isAddedDepartureTime = VehicleController.addDepartureTime(vehicle,txtDepartureTime.getText(),txtCarNumber.getText());
            if (isAddedDepartureTime){
                vehicle.setSQL("Update carParking set departureTime=? where number=? order by cID desc limit 1");
                VehicleController.addDepartureTime(vehicle,txtDepartureTime.getText(),txtCarNumber.getText());
    ...
    public void btnPayOnAction(ActionEvent actionEvent) throws Exception {
        Vehicle vehicle = new Car();
        vehicle.setSQL("Update carParkingTempory set parkingFee = ? where number = ?");
        try {
            boolean isAddedPayment = VehicleController.addParkingPayment(vehicle,txtPaymentField.getText(),txtCarNumber.getText());
            if (isAddedPayment){
                vehicle.setSQL("Update carParking set parkingFee = ? where number = ? order by cID desc limit 1");
                VehicleController.addParkingPayment(vehicle,txtPaymentField.getText(),txtCarNumber.getText());
    ...
    public void searchFieldSet(){
        Vehicle vehicle = new Car();
        vehicle.setSQL("Select number,firstName,lastName,arrivalTime From carParkingTempory where number=?");
        try {
            Vehicle v = VehicleController.getParkingHistoryforUpdateDeparture(vehicle,txtSearchNumber.getText(),"Car");
            if (v!=null){
    ...
    public void rbCashOnAction(ActionEvent actionEvent) {
        Vehicle vehicle = new Car();
        vehicle.setSQL("Update carParkingTempory set paymentStatus = ? where number = ?");
        try {
            boolean isAddedPaymentStatus = VehicleController.addPaymentStatus(vehicle,"Payed via CASH",txtCarNumber.getText());
            if (isAddedPaymentStatus){
                vehicle.setSQL("Update carParking set paymentStatus = ? where number = ? order by cID desc limit 1");
                VehicleController.addPaymentStatus(vehicle,"Payed via CASH",txtCarNumber.getText());
    ...
    public void btnPay2OnAction(ActionEvent actionEvent) {
        ...
        vehicle.setSQL("Update carParkingTempory set paymentStatus = ? where number = ?");
        try {
            boolean isAddedPaymentStatus = VehicleController.addPaymentStatus(vehicle,"Payed via VISA CARD",txtCarNumber.getText());
            if (isAddedPaymentStatus){
                vehicle.setSQL("Update carParking set paymentStatus = ? where number = ? order by cID desc limit 1");
                VehicleController.addPaymentStatus(vehicle,"Payed via VISA CARD",txtCarNumber.getText());
                txtThankYou.setVisible(true);
