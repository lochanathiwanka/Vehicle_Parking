...
public class UpdateBusDepartureTimeFormController {
    ...
    public void btnAddOnAction(ActionEvent actionEvent) {
        ...
        vehicle.setSQL("Update busParkingTempory set departureTime=? where number=?");
        try {
            ...
            double setFee = Double.parseDouble(totMinitues)*(100/60d);
            String totFee = String.format("%.2f",setFee);
            txtPaymentField.setText(totFee);

            boolean isAddedDepartureTime = VehicleController.addDepartureTime(vehicle,txtDepartureTime.getText(),txtBusNumber.getText());
            if (isAddedDepartureTime){
                vehicle.setSQL("Update busParking set departureTime=? where number=? order by bID desc limit 1");
                VehicleController.addDepartureTime(vehicle,txtDepartureTime.getText(),txtBusNumber.getText());
    ...
    public void btnPayOnAction(ActionEvent actionEvent) throws Exception {
        Vehicle vehicle = new Bus();
        vehicle.setSQL("Update busParkingTempory set parkingFee = ? where number = ?");
        try {
            boolean isAddedPayment = VehicleController.addParkingPayment(vehicle,txtPaymentField.getText(),txtBusNumber.getText());
            if (isAddedPayment){
                vehicle.setSQL("Update busParking set parkingFee = ? where number = ? order by bID desc limit 1");
                VehicleController.addParkingPayment(vehicle,txtPaymentField.getText(),txtBusNumber.getText());
    ...
    public void searchFieldSet(){
        Vehicle vehicle = new Bus();
        vehicle.setSQL("Select number,firstName,lastName,arrivalTime From busParkingTempory where number=?");
        try {
            Vehicle v = VehicleController.getParkingHistoryforUpdateDeparture(vehicle,txtSearchNumber.getText(),"Bus");
            if (v!=null){
                txtBusNumber.setText(v.getVehicleNumber());
    ...
    public void rbCashOnAction(ActionEvent actionEvent) {
        Vehicle vehicle = new Bus();
        vehicle.setSQL("Update busParkingTempory set paymentStatus = ? where number = ?");
        try {
            boolean isAddedPaymentStatus = VehicleController.addPaymentStatus(vehicle,"Payed via CASH",txtBusNumber.getText());
            if (isAddedPaymentStatus){
                vehicle.setSQL("Update busParking set paymentStatus = ? where number = ? order by bID desc limit 1");
                VehicleController.addPaymentStatus(vehicle,"Payed via CASH",txtBusNumber.getText());
    ...
    public void btnPay2OnAction(ActionEvent actionEvent) {
        ...
        vehicle.setSQL("Update busParkingTempory set paymentStatus = ? where number = ?");
        try {
            boolean isAddedPaymentStatus = VehicleController.addPaymentStatus(vehicle,"Payed via VISA CARD",txtBusNumber.getText());
            if (isAddedPaymentStatus){
                vehicle.setSQL("Update busParking set paymentStatus = ? where number = ? order by bID desc limit 1");
                VehicleController.addPaymentStatus(vehicle,"Payed via VISA CARD",txtBusNumber.getText());
                txtThankYou.setVisible(true);
