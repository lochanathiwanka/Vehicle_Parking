...
public class UpdateLorryDepartureTimeFormController {
    ...
    public void btnAddOnAction(ActionEvent actionEvent) {
        ...
        vehicle.setSQL("Update lorryParkingTempory set departureTime=? where number=?");
        try {
            ...
            double setFee = Double.parseDouble(totMinitues)*(150/60d);
            String totFee = String.format("%.2f",setFee);
            txtPaymentField.setText(totFee);

            boolean isAddedDepartureTime = VehicleController.addDepartureTime(vehicle,txtDepartureTime.getText(),txtLorryNumber.getText());
            if (isAddedDepartureTime){
                vehicle.setSQL("Update lorryParking set departureTime=? where number=? order by lID desc limit 1");
                VehicleController.addDepartureTime(vehicle,txtDepartureTime.getText(),txtLorryNumber.getText());
    ...
    public void btnPayOnAction(ActionEvent actionEvent) throws Exception {
        Vehicle vehicle = new Lorry();
        vehicle.setSQL("Update lorryParkingTempory set parkingFee = ? where number = ?");
        try {
            boolean isAddedPayment = VehicleController.addParkingPayment(vehicle,txtPaymentField.getText(),txtLorryNumber.getText());
            if (isAddedPayment){
                vehicle.setSQL("Update lorryParking set parkingFee = ? where number = ? order by lID desc limit 1");
                VehicleController.addParkingPayment(vehicle,txtPaymentField.getText(),txtLorryNumber.getText());
    ...
    public void searchFieldSet(){
        Vehicle vehicle = new Lorry();
        vehicle.setSQL("Select number,firstName,lastName,arrivalTime From lorryParkingTempory where number=?");
        try {
            Vehicle v = VehicleController.getParkingHistoryforUpdateDeparture(vehicle,txtSearchNumber.getText(),"Lorry");
            if (v!=null){
                txtLorryNumber.setText(v.getVehicleNumber());
    ...
    public void rbCashOnAction(ActionEvent actionEvent) {
        Vehicle vehicle = new Lorry();
        vehicle.setSQL("Update lorryParkingTempory set paymentStatus = ? where number = ?");
        try {
            boolean isAddedPaymentStatus = VehicleController.addPaymentStatus(vehicle,"Payed via CASH",txtLorryNumber.getText());
            if (isAddedPaymentStatus){
                vehicle.setSQL("Update lorryParking set paymentStatus = ? where number = ? order by lID desc limit 1");
                VehicleController.addPaymentStatus(vehicle,"Payed via CASH",txtLorryNumber.getText());
    ...
    public void btnPay2OnAction(ActionEvent actionEvent) {
        ...
        vehicle.setSQL("Update lorryParkingTempory set paymentStatus = ? where number = ?");
        try {
            boolean isAddedPaymentStatus = VehicleController.addPaymentStatus(vehicle,"Payed via VISA CARD",txtLorryNumber.getText());
            if (isAddedPaymentStatus){
                vehicle.setSQL("Update lorryParking set paymentStatus = ? where number = ? order by lID desc limit 1");
                VehicleController.addPaymentStatus(vehicle,"Payed via VISA CARD",txtLorryNumber.getText());
                txtThankYou.setVisible(true);
