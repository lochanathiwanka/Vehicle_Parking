...
public class BusOwnerPaymentCheckFormController implements Initializable {
    public TextField txtBusNumberField;
    ...
    public void btnRemoveOnAction(ActionEvent actionEvent) {
        ...
        vehicle.setSQL("Delete from busParkingTempory where number = ?");
        try{
            boolean isRemoved = VehicleController.removeVehicleFromParkingTempory(vehicle,txtBusNumberField.getText());
            if (isRemoved){
                JOptionPane.showMessageDialog(null,"Bus was removed from parking","",JOptionPane.INFORMATION_MESSAGE);
                textFieldsReset();
            }
    ...
    public void textFieldsReset(){
        txtBusNumberField.setText(null);
    ...
    public void forSearchField(){
        Vehicle vehicle = new Bus();
        vehicle.setSQL("Select number,firstName,lastName,paymentStatus from busParkingTempory where number = ?");
        try {
            String[] list = VehicleController.getAllValuesFromParkingHistory(vehicle,txtSearch.getText());
            if (list!=null){
                txtBusNumberField.setText(list[0]);
