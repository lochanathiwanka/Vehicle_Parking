...
public class BusSearchFormController {
    ...
    public void btnAddToParkingOnAction(ActionEvent actionEvent) {
        ...
        vehicle.setSQL("Insert into busParkingTempory (number,firstName,lastName,date,arrivalTime) Values(?,?,?,?,?)");
        try{
            boolean isAddedtoPark = VehicleController.addVehicletoPark(vehicle,txtDate.getText(),txtArrivalTime.getText());
            if (isAddedtoPark){
                JOptionPane.showMessageDialog(null, "Bus was Successfully added to the Parking","",JOptionPane.INFORMATION_MESSAGE);
                vehicle.setSQL("Insert into busParking (number,firstName,lastName,date,arrivalTime) Values(?,?,?,?,?)");
            ...
            JOptionPane.showMessageDialog(null, "Can't added Bus","",JOptionPane.WARNING_MESSAGE);
    ...
    public void findIsVehicleInParking(){
        Vehicle v = new Bus();
        v.setSQL("Select number from busParkingTempory where number =?");
        try {
