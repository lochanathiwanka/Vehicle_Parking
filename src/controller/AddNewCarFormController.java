...
public class AddNewCarFormController {
    ...
    public void btnAddOnAction(ActionEvent actionEvent) {
        ...
        vehicle.setSQL("Insert into carDetails (number,firstName,lastName) Values(?,?,?)");
        try {
            boolean isCarAdded = VehicleController.addVehicle(vehicle);
            if (isCarAdded){
                JOptionPane.showMessageDialog(null,"Car was registered to the system","",JOptionPane.INFORMATION_MESSAGE);
                clearTextFields();
            }
        }catch (ClassNotFoundException ex){
            JOptionPane.showMessageDialog(null,"Error","",JOptionPane.ERROR_MESSAGE);
        }catch (SQLException ex){
            JOptionPane.showMessageDialog(null,"Car is already added to the system","",JOptionPane.WARNING_MESSAGE);
        }
    }
    ...
    public void forSearchField(){
        ...
        try {
            Vehicle vehicle=VehicleController.searchVehicle(txtCarNumber.getText(),"Car",v);
            if(vehicle!=null){
